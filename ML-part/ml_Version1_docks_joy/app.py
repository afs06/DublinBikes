...
model = None
model_path_joblib = 'bike_docks_model_v1.joblib'
model_path_pkl = 'bike_docks_model_v1.pkl'
...
def home():
    ...
    return render_template('parking.html', station_ids=station_ids)

# API endpoint to get predictions for a specific station
@app.route('/predict_docks/<int:station_id>')
def predict_docks(station_id):
    try:
        ...
        for hour in range(24):
            ...
            if model is not None:
                ...
                prediction = 15 + (5 - hour % 10)  # Simple pattern for testing
            
            predictions.append({
                'hour': hour,
                'predicted_docks': prediction
            })
...
if __name__ == '__main__':
    app.run(debug=True, port=5001)  # Using port 5001 to avoid conflict with bike app